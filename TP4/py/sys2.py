...
for i in range(len(omegas)):
    ...
    amplitudes2500.append(df['a'].tolist())
...
def graphAll(maxes1,maxes2,maxes3,maxes4,maxes5,omegas):
    ...
    plt.subplot(omegas, maxes2, color='blue', label='k = 1000')
    plt.subplot(omegas, maxes3, color='green', label='k = 2000')
    plt.subplot(omegas, maxes4, color='yellow', label='k = 5000')
    plt.subplot(omegas, maxes5, color='purple', label='k = 10000')
    plt.xlabel('Omega [rad/s]')
    plt.ylabel('Amplitude [m]')
    plt.legend()
...
graphOnlyOne(max_amplitudes2500,omegas,2500)
