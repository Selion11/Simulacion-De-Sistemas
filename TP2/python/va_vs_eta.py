...
for eta in etas:
    ...
    for N in Ns:
        ...
        with open(filename, 'r') as file:
            ...
            Va_promedio = np.mean(Va_values[t_start-1:t_end])  

            Va_desvio = np.std(Va_values[t_start-1:t_end])

            # Guardar el promedio en el diccionario
            promedios_Va[N].append(Va_promedio)
            desvios_Va[N].append(Va_desvio)

# Graficar Va vs eta para cada N con desvio
plt.figure(figsize=(10, 6))
# for N in Ns:
plt.errorbar(etas,promedios_Va[400],yerr=desvios_Va[400],fmt='o', label=f'N={N}',capsize=5,color='red')
...
plt.xlabel('η (ruido)',fontsize=16)
plt.ylabel('Promedio de Va',fontsize=16)
# plt.legend()
plt.grid(True)
plt.show()